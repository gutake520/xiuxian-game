...
export function techniqueLibrary(api,onClose){
 ...
 body.innerHTML=(fragments>0&&fragments<3?`<p>残缺玉简 ${fragments}/3</p>`:'')+(methods.length?methods.map(m=>{const learned=save.techniques.mastered.includes(m.id),main=save.techniques.main===m.id,equipped=save.techniques.combat?.includes(m.id);return `<article class="xg-feature-card"><h3>${m.name}<small>${learned?'已学会':'待参悟'}${main?' · 主修':''}${equipped?' · 已装备':''}</small></h3><p>${m.description}</p><button type="button" data-method="${m.id}" data-learned="${learned}">${learned?m.type==='combat'?(equipped?'卸下战斗功法':'装备战斗功法'):m.type==='craft'?'已学会':main?'卸下主修':'设为主修':'参悟数阵'}</button></article>`}).join(''):'<p>尚无典籍。可在坊市购买，或拜入宗门领取。</p>');
