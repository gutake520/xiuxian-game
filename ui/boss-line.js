...
export function showBossStory({getSave,actions,activate,onDone}){
 ...
 if(line.phase==='ambush'){
  ...
  content.innerHTML=`<section class="xg-map-sheet xg-battle-sheet"><h2>故人衣纹</h2><div class="xg-card"><p>炼气十层。你刚稳住气息，便在山道尽头认出了那夜的衣纹。</p><p>${BOSS_NAME}转过身，力量远胜于你。</p><p>对手生命 ${foe.maxHp.toFixed(2)} · 攻击 ${foe.attack.toFixed(2)} · 防御 ${foe.defense.toFixed(2)} · 速度 ${foe.speed.toFixed(2)}</p></div><button type="button" data-boss-story>迎战</button><p role="status"></p></section>`;
 }else{
  content.innerHTML=`<section class="xg-map-sheet xg-battle-sheet"><h2>山道余生</h2><div class="xg-card"><p>你被${BOSS_NAME}打倒，连握剑的力气也快没了。</p><p>${escapeHTML(line.elder)}赶来救下你，将${BOSS_NAME}打成重伤。对方逃入丰原镇附近的山中养伤。</p></div><button type="button" data-boss-story>前往丰原镇</button><p role="status"></p></section>`;
 }
