...
export function showSectTasks(api,onClose){
 ...
 body.innerHTML=`<p>每日随机三项，每项奖励 1 积分 · 当前积分 ${save.sectPoints||0}</p>${DAILY_TASKS.filter(task=>daily.selected?.includes(task.id)).map(task=>`<article class="xg-feature-card"><h3>${task.name}</h3><p>${daily[task.id]||0} / ${task.target}</p><button type="button" data-claim="${task.id}" ${daily.claimed.includes(task.id)||(daily[task.id]||0)<task.target?'disabled':''}>${daily.claimed.includes(task.id)?'已领取':'领取 1 积分'}</button></article>`).join('')}<small>按本地日期更新。逃跑扣款不计入消费；五色灵境任务只计算小游戏结算的修为。</small>`;
