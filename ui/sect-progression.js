...
export function showSectWorkshop(api,sectId,onClose){
 ...
 if(sectId==='xuanji')body.innerHTML='<p>定身阵盘：消耗 20 份矿石，使用六次后损坏。每次发动使对手下一轮无法行动。</p><button type="button" data-craft="crafted-binding-array">制作自制定身阵盘</button>';
 if(sectId==='wanling'){
  const beasts=ensureSpiritBeasts(save),herbs=['healing-herb','spirit-herb','qi-herb'];
  body.innerHTML=`<p>入门获赠两只灵兽，可分别取名。学会《铲屎官手册》后，每只每日消耗 3 株对应境界的药草，喂过的灵兽当天可以出战。</p>${BEAST_TYPES.map(type=>{const beast=beasts[type],fed=beastCanFight(save,type);return `<article class="xg-feature-card"><h3>${escapeHTML(beast.name)} · ${type==='attack'?'追击':'守护'}</h3><p>${escapeHTML(beast.stage)} · ${fed?'今日已喂养，可出战':'今日尚未喂养'} · ${type==='attack'?'每轮追加 0.50 伤害':'每轮抵挡 0.30 伤害'}</p><label>取名 <input type="text" data-beast-name="${type}" maxlength="12" value="${escapeHTML(beast.name)}"></label><button type="button" data-rename="${type}">保存名字</button>${fed?'':beast.stage==='筑基'?'<p>需筑基期灵草；材料尚未开放。</p>':hasActiveTechnique(save,'beast-keeper')?`<label>选择药草 <select data-feed-herb="${type}">${herbs.map(id=>`<option value="${id}">${ITEMS[id].name}</option>`).join('')}</select></label><button type="button" data-feed="${type}">喂养 · 3 株</button>`:'<p>学会《铲屎官手册》后可喂养。</p>'}</article>`}).join('')}`;
 }
 if(sectId==='tiangong'){
  const gear=save.inventory.filter(entry=>ITEMS[entry.itemId]?.kind==='equipment');
  body.innerHTML=`<p>修补费用按耐久上限每 5 点收 1 个矿石，不论掉了多少耐久，直接修满。</p>${gear.length?gear.map(entry=>{const limit=maxDurability(entry),cost=Math.ceil(limit/5);return `<article class="xg-feature-card"><h3>${escapeHTML(ITEMS[entry.itemId].name)}</h3><p>耐久 ${entry.durability??limit}/${limit} · 消耗矿石 ${cost}</p><button type="button" data-repair="${escapeHTML(entry.uid)}" ${entry.durability>=limit?'disabled':''}>${entry.durability>=limit?'已修满':'修补至满耐久'}</button></article>`}).join(''):'<p>储物中没有装备。</p>'}`;
 }
