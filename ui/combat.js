...
export function showBattle({getSave,actions,activate,onExit},endResult=null){
 ...
 if(!battle){
  ...
  const returnText=['sect-tournament','sect-senior'].includes(result?.kind)?'返回宗门':'返回丰原镇';
  ...
  content.innerHTML=`<section class="xg-map-sheet xg-battle-sheet"><h2>战斗结束</h2><p>${result?.outcome==='victory'?'击败对手':result?.outcome==='defeat'?'你败下阵来':'已脱离战斗'}</p><div class="xg-card">${(result?.log||[]).map(line=>`<p>${escapeHTML(line)}</p>`).join('')}</div>${reward}${divinationMarkup(diviner,result?.divinationResult)}${!diviner&&encounter?`<div class="xg-card"><h3>${escapeHTML(HERBALIST_NAME)}</h3><p>他身上带伤，想向你讨一株回血草。</p><p>已获玉简 ${save.herbalistFragments||0}/3</p><div class="xg-feature-row"><button type="button" data-encounter-give ${hasHerb?'':'disabled'}>交出一株回血草</button><button type="button" data-encounter-leave>离开</button></div></div>`:diviner||senior?'':`<button type="button" data-return>${returnText}</button>`}<p role="status" aria-live="polite"></p></section>`;
  content.querySelector('[data-flip-coin]')?.addEventListener('click',async event=>{const button=event.currentTarget;button.disabled=true;try{const {result:flipped}=await actions.flipDivination(diviner.id);const coin=content.querySelector('.xg-divination-coin');coin.classList.add(flipped.front?'is-front':'is-back');let shown=false;const finish=()=>{if(shown)return;shown=true;coin.innerHTML=coinFace(flipped.front);showBattle({getSave,actions,activate,onExit})};coin.addEventListener('animationend',finish,{once:true});if(matchMedia('(prefers-reduced-motion: reduce)').matches)setTimeout(finish,30)}catch(error){content.querySelector('[role=status]').textContent=error.message;button.disabled=false}});
