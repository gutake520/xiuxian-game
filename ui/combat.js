...
export function showBattle({getSave,actions,activate,onExit},endResult=null){
 ...
 content.innerHTML=`<section class="xg-map-sheet xg-battle-sheet"><h2>${escapeHTML(battle.name)}</h2><p>第 ${battle.round+1} 轮 · ${stats.speed>=battle.speed?'你先手':'小妖先手'}</p>
  ...
  ${(save.techniques?.combat||[]).length?`<div class="xg-battle-talisman"><strong>战斗功法</strong>${battle.criticalFocus?'<small>凝神中 · 本场暴击率 +15%</small>':''}<div class="xg-battle-talisman-actions">${save.techniques.combat.filter(id=>!TECHNIQUES[id]?.passive).map(id=>{const skill=TECHNIQUES[id],wait=Math.max(0,(battle.skillReady?.[id]||0)-battle.round-1),used=id==='only-once'&&battle.criticalFocus;return skill?`<button type="button" data-fight="${id}" ${used||wait||save.player.mp<(skill.mpCost??1)?'disabled':''}>${skill.name}${used?' · 本场已用':wait?' · 冷却 '+wait+' 轮':''}</button>`:''}).join('')}</div></div>`:''}
