...
export function showBattle({getSave,actions,activate,onExit},endResult=null){
 ...
 if(!battle){
  ...
  content.innerHTML=`<section class="xg-map-sheet xg-battle-sheet"><h2>战斗结束</h2><p>${result?.outcome==='victory'?'击败对手':result?.outcome==='defeat'?'你败下阵来':'已脱离战斗'}</p><div class="xg-card">${(result?.log||[]).map(line=>`<p>${escapeHTML(line)}</p>`).join('')}</div><button type="button" data-return>返回丰原镇</button></section>`;
