...
export function showBattle({getSave,actions,activate,onExit},endResult=null){
 ...
 content.innerHTML=`<section class="xg-map-sheet xg-battle-sheet"><h2>${escapeHTML(battle.name)}</h2><p>第 ${battle.round+1} 轮 · ${stats.speed>=battle.speed?'你先手':'小妖先手'}</p>
  ...
  <div class="xg-battle-action-panel"><h3>本轮行动</h3><div class="xg-battle-main-actions"><button type="button" data-fight="attack">普攻</button><button type="button" data-fight="skip">跳过</button></div>
  ${(save.techniques?.combat||[]).length?`<div class="xg-battle-talisman"><strong>战斗功法</strong><div class="xg-battle-talisman-actions">${save.techniques.combat.map(id=>{const skill=TECHNIQUES[id],wait=Math.max(0,(battle.skillReady?.[id]||0)-battle.round-1);return skill?`<button type="button" data-fight="${id}" ${wait?'disabled':''}>${skill.name}${wait?' · 冷却 '+wait+' 轮':''}</button>`:''}).join('')}</div></div>`:''}
