...
import {purchase,equipItem,ownsTechnique} from '../systems/inventory.js';
...
function showItem(api,uid){
 ...
 body.innerHTML=`<p>${escapeHTML(item?.description||'这件旧物已保留，暂不可使用。')}</p>${item?.kind==='manual'?'<button type="button" data-use>阅读典籍</button>':item?.kind==='equipment'?`<button type="button" data-use>${Object.values(save.equipment).includes(uid)?'卸下装备':'装备'}</button>`:''}`;
