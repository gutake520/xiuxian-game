...
import {purchase,equipItem,ownsTechnique,claimLoot,sellEquipment,discardEquipment,usePill,equipmentSalePrice} from '../systems/inventory.js';
...
function showShop(api,gearOnly=false){
 const sheet=createSheet(gearOnly?'购买装备':'坊市商店',()=>{api.closeFeature();renderInventory(api)}),body=sheet.querySelector('[data-body]'),message=sheet.querySelector('[role=status]');
 ...
 body.querySelectorAll('[data-buy]').forEach(button=>button.onclick=()=>buttonTask(button,async()=>{await api.actions.mutate(s=>purchase(s,button.dataset.buy),{message:result=>result});if(!sheet.isConnected)return;showShop(api,gearOnly);document.querySelector('#xg-feature-sheet [role=status]').textContent='已收入储物格。'},message));
}
function showSell(api){
 const sheet=createSheet('出售装备',()=>{api.closeFeature();renderInventory(api)}),body=sheet.querySelector('[data-body]'),message=sheet.querySelector('[role=status]');
 const gear=api.getSave().inventory.filter(entry=>ITEMS[entry.itemId]?.kind==='equipment');
 body.innerHTML=gear.length?gear.map(entry=>{const item=ITEMS[entry.itemId],full=entry.durability===20;return `<article class="xg-feature-card"><h3>${escapeHTML(item.name)}<small>耐久 ${format(entry.durability)} / 20</small></h3><p>${full?`满耐久可卖 ${equipmentSalePrice(item)} 灵石。`:'耐久未满，只能丢弃；修补费用待定。'}</p>${full?`<button type="button" data-sale="${escapeHTML(entry.uid)}">出售</button>`:''}<button type="button" data-discard="${escapeHTML(entry.uid)}">丢弃</button></article>`}).join(''):'<p>没有可以出售的装备。</p>';
 body.querySelectorAll('[data-sale]').forEach(button=>button.onclick=()=>buttonTask(button,async()=>{await api.actions.mutate(s=>sellEquipment(s,button.dataset.sale),{message:result=>result});if(sheet.isConnected)showSell(api)},message));
 body.querySelectorAll('[data-discard]').forEach(button=>button.onclick=()=>buttonTask(button,async()=>{if(!confirm('丢弃这件装备？丢弃后无法找回。'))return;await api.actions.mutate(s=>discardEquipment(s,button.dataset.discard));if(sheet.isConnected)showSell(api)},message));
}
function showItem(api,uid){
 ...
 body.querySelector('[data-sale]')?.addEventListener('click',e=>buttonTask(e.currentTarget,async()=>{await api.actions.mutate(s=>sellEquipment(s,uid),{message:result=>result});if(sheet.isConnected)back()},message));
