...
import {idleRate} from '../systems/cultivation.js';
...
export function showCultivation(api,onClose){
 ...
 body.innerHTML=`<div class="xg-feature-card"><h3>${escapeHTML(save.player.realm)}</h3><p>修为 ${format(progress.current)} / ${progress.required??'圆满'}</p><p>主修：${main?.name||'未装备'}</p><p>挂机收益：${format(idleRate(save))} / 分钟</p><p>今日已挂机 ${format(save.idle.usedMs/60000)} / ${IDLE_LIMIT_MS/60000} 分钟</p><small>按本机现实日期零点重置。主修装备期间自动积累，离线同样有效。</small></div><div class="xg-feature-row"><button type="button" data-main>选择主修</button><button type="button" data-refresh>结算挂机</button></div><div class="xg-practice-intro"><h3>五色灵境</h3><p>拖动接灵台，让灵珠击碎上方灵气。根骨越高，接灵台越宽。</p><small>${main?'每块灵气 +2 修为，清场最多 60；有三次失手机会。':'尚未装备主修：本局消除至少一块后，只能获得基础修为 +1。'}</small><button type="button" data-start ${progress.complete||progress.index<0?'disabled':''}>进入灵境</button></div>${save.techniques.mastered.some(id=>UPGRADEABLE_TECHNIQUES.includes(id))?`<div class="xg-feature-card"><h3>功法精进</h3><p>施放满 50 次，可花 20 灵石开启四阶数阵。</p>${UPGRADEABLE_TECHNIQUES.filter(id=>save.techniques.mastered.includes(id)).map(id=>{const times=save.techniques.usage?.[id]||0,done=save.techniques.upgraded?.includes(id),pending=Boolean(save.techniques.puzzles?.['upgrade:'+id]);return `<div class="xg-method-row"><span>${escapeHTML(TECHNIQUES[id].name)} · ${done?'中级':Math.min(50,times)+'/50 次'}</span>${done?'<small>已精进</small>':`<button type="button" data-upgrade="${id}" ${!pending&&(times<50||save.player.spiritStones<20)?'disabled':''}>${pending?'继续参悟':'精进 · 20 灵石'}</button>`}</div>`}).join('')}</div>`:''}${progress.complete?'<p>炼气已圆满，筑基突破暂未开放。</p>':''}`;
