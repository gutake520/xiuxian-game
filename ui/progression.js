...
export function progressMarkup(player){const p=realmProgress(player),value=p.required?Math.max(0,Math.min(100,p.current/p.required*100)):p.complete?100:0;return `<div class="xg-cultivation"><span>修为</span><strong>${p.complete?'炼气圆满':`${format(p.current)} / ${p.required??'—'}`}</strong></div><div class="xg-progress" role="progressbar" aria-label="修为进度" aria-valuenow="${value}" aria-valuemin="0" aria-valuemax="100"><i style="width:${value}%"></i></div><div class="xg-cultivation"><span>灵石</span><strong>${format(player.spiritStones)}</strong></div>`}
