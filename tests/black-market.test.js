...
test('ten draws cost eighteen, pity persists and forced rewards reset it; replay does not charge',()=>{
 ...
 assert.equal(s.player.spiritStones,82);assert.equal(s.blackMarket.pity,9);assert.equal(s.inventory.find(e=>e.itemId==='one-manual').quantity,1);assert.deepEqual(s.blackMarket.lastDraw.manuals,['这里有一']);
...
test('both rare manuals carry their own reveal names',()=>{
 const first=save();random(0,()=>drawBlackMarket(first,1,'first'));assert.deepEqual(first.blackMarket.lastDraw.manuals,['这里有一']);
 const second=save();second.blackMarket={pity:65};random(.9,()=>drawBlackMarket(second,1,'second'));assert.deepEqual(second.blackMarket.lastDraw.manuals,['滴，重置卡']);
});
