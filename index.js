...
async function loadGame(){const last=localStorage.getItem(LAST_SLOT_KEY);if(last){try{const s=await dbGet(last);if(s){await game.select(last);renderHome();return}}catch(error){console.error('[xiuxian-game] 当前存档读取失败',error)}}await showSlots()}
