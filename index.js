import {attachViewportPanel} from './ui/viewport.js';
import {showAchievements} from './ui/achievements.js';
import {sectExitPrice,leaveSect,assertCanJoinSect} from './systems/sect-membership.js';
...
function renderHome(){
 ...
 <div class="xg-card xg-daily"><h3>宗门日课</h3><p>${inSect?DAILY_TASKS.filter(task=>dailyTasks(currentSave).selected?.includes(task.id)).map(task=>`${task.name} ${dailyTasks(currentSave)[task.id]||0}/${task.target}${dailyTasks(currentSave).claimed.includes(task.id)?' · 已领取':''}`).join('<br>'):'尚未入宗，暂无宗门日课。'}</p></div>
...
function renderCharacter(){
 ...
 if(currentSave.bossLine?.phase==='ambush'||currentSave.bossLine?.rescuePending)return mapUI.render();
 activatePage('person');
...
function showSect(){
 ...
 if(['sect-tournament','sect-senior'].includes(currentSave.battle?.kind))return showSectTournamentBattle(sect);
 ...
 sheet.innerHTML=`<div class="xg-sect-heading"><h2>择宗入道</h2><button type="button" id="xg-sect-back">返回人物</button></div><p class="xg-sect-hint">择一山门，寻一条修行路。变异灵根可入各宗，特色传承仍需满足专精条件。</p>${(currentSave.sectRejoinAt||0)>Date.now()?`<p>离宗冷静期：还需 ${Math.ceil((currentSave.sectRejoinAt-Date.now())/3600000)} 小时。</p>`:''}<div class="xg-sect-list">${SECTS.map(sect=>{const eligible=sectEligibility(currentSave.player,sect);return `<button type="button" data-sect="${sect.id}" ${eligible.join&&(currentSave.sectRejoinAt||0)<=Date.now()?'':'disabled'}><strong>${sect.name}</strong><small>${sect.condition}</small><span>${sect.feature}</span><em>${eligible.join?(eligible.specialty?'可入宗 · 可学特色传承':'可入宗 · 仅通用功法'):'暂不符合条件'}</em></button>`}).join('')}</div>`;
...
function previewSect(sect){
 ...
 document.getElementById('xg-sect-join').onclick=async()=>{
  ...
  try{await joinSect(sect.id);renderSectIntro(sect)}catch(error){console.error('[xiuxian-game]',error);document.getElementById('xg-sect-error').textContent=error.message||'拜师未完成，请重试。';button.disabled=back.disabled=false}
 };
 sheet.scrollTop=0;
}
let sectBusy=false;
...
function showSectTournamentBattle(sect){
 document.getElementById('xg-sect-sheet')?.classList.remove('open');
 showBattle({getSave:()=>currentSave,actions:game,activate:activatePage,onExit:()=>renderSectHall(sect)});
}
...
function renderSectHall(sect){
 ...
 sheet.querySelector('[data-sect-tournament]').onclick=async()=>{const button=sheet.querySelector('[data-sect-tournament]');button.disabled=true;try{await game.startSectTournament(sheet.querySelector('[name="xg-sect-pet"]:checked')?.value||null);showSectTournamentBattle(sect)}catch(error){sheet.querySelector('[data-leave-status]').textContent=error.message;button.disabled=false}};
...
document.getElementById('xg-bag').onclick=()=>runAction(async()=>{if(!currentSave)return showSlots();if((currentSave?.qiSecret||currentSave?.qiMeditation||currentSave?.bossLine?.phase==='ambush'||currentSave?.bossLine?.rescuePending))return mapUI.render();await game.refresh();featureUI.inventory()});
document.getElementById('xg-map').onclick=()=>{if(currentSave)mapUI.render();else runAction(showSlots)};
const collapse=()=>{featureUI.close();document.getElementById('xg-sect-sheet')?.classList.remove('open');panel.classList.remove('open');document.getElementById('xg-settings-sheet').classList.remove('open');document.getElementById('xg-slots').classList.remove('open');document.getElementById('xg-onboard').classList.remove('open');fab.classList.remove('hide')};document.getElementById('xg-head-moon').onclick=collapse;document.getElementById('xg-settings').onclick=()=>(currentSave?.qiSecret||currentSave?.qiMeditation||currentSave?.bossLine?.phase==='ambush'||currentSave?.bossLine?.rescuePending)?mapUI.render():document.getElementById('xg-settings-sheet').classList.add('open');document.getElementById('xg-achievements').onclick=()=>runAction(async()=>{if(!currentSave){alert('请先选择存档。');return}await game.refresh();document.getElementById('xg-settings-sheet').classList.remove('open');showAchievements({getSave:()=>currentSave,actions:game},()=>{document.getElementById('xg-feature-sheet')?.remove();document.getElementById('xg-settings-sheet').classList.add('open')})});document.getElementById('xg-settings-close').onclick=()=>document.getElementById('xg-settings-sheet').classList.remove('open');document.getElementById('xg-slots-btn').onclick=()=>{document.getElementById('xg-settings-sheet').classList.remove('open');runAction(showSlots)};document.getElementById('xg-slots-close').onclick=()=>document.getElementById('xg-slots').classList.remove('open');document.getElementById('xg-save').onclick=()=>runAction(saveNow);document.getElementById('xg-update').onclick=()=>runAction(async()=>{await saveNow();location.reload()})}
