...
export function startLearning(save,id){
 const method=TECHNIQUES[id];if(!method||!hasManual(save,id))throw new Error('请先取得对应典籍。');
...
export function writePuzzleCell(save,id,index,value){
 const puzzle=save.techniques.puzzles[id];if(!puzzle||!hasManual(save,id))throw new Error('学习记录不存在。');
...
export function useHint(save,id){
 const puzzle=save.techniques.puzzles[id];if(!puzzle||!hasManual(save,id))throw new Error('请先开启学习。');
...
export function completeLearning(save,id){
 const puzzle=save.techniques.puzzles[id];if(!puzzle||save.techniques.mastered.includes(id)||!hasManual(save,id))throw new Error('当前无法结算学习。');
...
export function setMainTechnique(save,id){
 if(id!==null&&(!TECHNIQUES[id]||!save.techniques.mastered.includes(id)))throw new Error('须先学会，再设为主修。');
