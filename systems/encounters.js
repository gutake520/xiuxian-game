...
export const HERBALIST_NAME='闻照';
...
export function resolveHerbalist(save,id,give){
 ...
 if(!give){save.encounterPending=null;return `你与${HERBALIST_NAME}暂别，继续赶路。`}
 ...
 if(save.herbalistFragments<MAX_FRAGMENTS)return `${HERBALIST_NAME}收下回血草，赠你一片残缺玉简（${save.herbalistFragments}/${MAX_FRAGMENTS}）。`;
 addItem(save,'spirit-burn-manual');
 return `${HERBALIST_NAME}交出最后一片玉简，三片合为《灵息尽燃》，典籍已收入储物。`;
}
